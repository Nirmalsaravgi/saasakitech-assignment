...
describe('getAverageClose', () => {
    ...
    it('should return the average close price when valid data is provided', async () => {
        StockData.aggregate.mockReturnValue(Promise.resolve([{ average_close: 110.5 }]));

        await getAverageClose(req, res);

        expect(res.json).toHaveBeenCalledWith({ average_close: 110.5 });
    });
